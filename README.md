...
- A WHMCS invoice's "Pay Now" button posts to `modules/gateways/nakopay/payment.php`, which calls `POST https://api.nakopay.com/v1/invoices-create` and shows the customer an address + QR + amount. The base URL is configurable via the "API Base URL override (advanced)" gateway setting or the `NAKOPAY_API_BASE` PHP constant; the Supabase functions URL is kept as a documented fallback.
...
[WHMCS](https://www.whmcs.com/) - web hosting billing and automation platform. Visit their website to learn more about the platform and its features.
