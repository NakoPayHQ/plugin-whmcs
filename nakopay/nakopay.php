...
class Client
{
     ...
     *
     * Resolution order:
     *   1) Gateway setting "ApiBaseUrl" (advanced field, blank = default)
     *   2) PHP constant NAKOPAY_API_BASE (host-defined)
     *   3) self::BASE_PRIMARY
     */
    const BASE_PRIMARY  = 'https://api.nakopay.com/v1/';
    const BASE_FALLBACK = 'https://daslrxpkbkqrbnjwouiq.supabase.co/functions/v1/';
    /** @deprecated since 0.3.0 - kept for backward compatibility with external callers. */
    const BASE_URL      = self::BASE_PRIMARY;
    ...
    {
        // 1) Per-install override via gateway config (blank = default).
        try {
            $params = $this->getGatewayParams();
            $override = trim((string) ($params['ApiBaseUrl'] ?? ''));
            if ($override !== '') {
                return rtrim($override, '/') . '/';
            }
        } catch (Exception $e) {
            // gateway not activated yet - fall through to constant / primary
        }
        // 2) Host-defined PHP constant (wp-config-style escape hatch).
